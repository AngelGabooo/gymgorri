...
const saveSales = (sales) => {
  localStorage.setItem(
    SALES_KEY,
    JSON.stringify(Array.isArray(sales) ? sales : [])
  );
...
export const createSale = ({
  ...
  const normalizedItems = items.map(item => {
    ...
    if (product.status === 'inactive') {
      throw new Error(`${product.name} está desactivado.`);
    }
  ...
  // Primero validar todo, después descontar stock.
  ...
  const sale = {
    id: saleId,
    folio: `VTA-${String(Date.now()).slice(-8)}`,
    customer: customer
      ? {
    ...
    total,
    estimatedCost: normalizedItems.reduce(
      (sum, item) =>
...
export const cancelSale = (saleId, reason = '') => {
  ...
  if (sales[index].status === 'cancelled') {
    return sales[index];
  }
  ...
  sales[index].items.forEach(item => {
    registerInventoryMovement({
      ...
      type: 'return',
      quantity: item.quantity,
      reason: `Cancelación de ${sales[index].folio}`,
      referenceId: saleId
    });
  });
...
export const getSalesSummary = (sales = getSales()) => {
  const completed = sales.filter(item => item.status !== 'cancelled');
  ...
  const todaySales = completed.filter(item => isToday(item.createdAt));
  const monthSales = completed.filter(item => isThisMonth(item.createdAt));

  const sum = list =>
    list.reduce((total, sale) => total + Number(sale.total || 0), 0);

  const profit = list =>
    list.reduce(
      (total, sale) => total + Number(sale.estimatedProfit || 0),
      0
    );
  ...
  return {
    ...
    todayIncome: sum(todaySales),
    monthIncome: sum(monthSales),
    todayProfit: profit(todaySales),
    monthProfit: profit(monthSales),
    productsSoldToday: todaySales.reduce(
      (sumValue, sale) => sumValue + Number(sale.itemCount || 0),
      0
    ),
    productsSoldMonth: monthSales.reduce(
      (sumValue, sale) => sumValue + Number(sale.itemCount || 0),
      0
    )
  };
